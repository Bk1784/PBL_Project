...
<div class="flex flex-col lg:flex-row gap-6">
    ...
    <div class="flex-1">
        <div class="flex space-x-4 overflow-x-auto p-4 bg-gray-200 rounded-lg">
            @php
                // Ambil produk kategori Dekorasi
                $products = App\Models\Dekorasi::where('slug', 'dekorasi')
                            ->where('status', '1')
                            ->latest()
                            ->get();
            @endphp
            ...
            <div class="grid grid-cols-3 gap-3 w-full">
                @foreach ($products as $dekorasi)
                <div class="p-2 bg-white rounded-lg shadow hover:scale-105 hover:shadow-md transform transition">
                    <a href="{{ route('detail_products', $dekorasi->id) }}" class="cursor-pointer block h-full">
                        <img src="{{ $dekorasi->image }}" alt="{{ $dekorasi->name }}" class="w-full h-20 object-cover rounded">
                        <h3 class="text-xs font-semibold mt-1 line-clamp-1">{{ $dekorasi->name }}</h3>
                        <p class="text-xs text-gray-500">⭐⭐⭐⭐☆ (4.5)</p>
                        <p class="text-xs font-bold text-blue-500">Rp{{ number_format($dekorasi->price, 0, ',', '.') }}</p>
                    </a>
                </div>
                @endforeach
            </div>
